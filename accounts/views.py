    ...
    try:
        if request.method =='POST':
            ...
            if not userexists:
                ...
                if not emailexits:
                    print('Email VErified',username,password,passwordconfirm)
                    if password==passwordconfirm and len(password) >=8 and password != username:
                        usernam=username.lower()
                        myuser=User.objects.create_user(usernam,email,passwordconfirm)
                        myuser.is_active=True
                        myuser.save()
                        current_site = get_current_site(request)
                        mail_subject = 'Confirm your Tambooz account.'
                        messagecon ={              
                        'user': myuser,
                        'domain': current_site.domain,
                        'uid':urlsafe_base64_encode(force_bytes(myuser.pk)),
                        'token':account_activation_token.make_token(myuser),}
                        message=get_template("home/acc_active_email.html").render(messagecon)
                        to_email = email
                        sender="Team Tambooz"
